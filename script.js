...
contactForm.addEventListener('submit', (event) => {
  ...
  fetch("https://script.google.com/macros/s/AKfycbzQYSTKumBHr0dpp7S1XDHZX40bWiY20i88x7EqPyLX5JWiPm-Dta8Xu6PzrqVC80CNjg/exec", {
    method: "POST",
    body: formData,
    mode: 'no-cors'
  })
  .then(res => res.text())
  .then(response => {
    msgStatus.textContent = 'Thank you, your message has been submitted!';
    msgStatus.style.color = '#16a34a';
    contactForm.reset();
  })
  .catch(error => {
    msgStatus.textContent = 'Error! Please try again.';
    msgStatus.style.color = '#dc2626';
